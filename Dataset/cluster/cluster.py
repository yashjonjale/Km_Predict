...
def plot_elbow_curve(data, max_clusters, num_iterations=100, batch_size=1000, num_processes=4, device='cuda'):
    with Pool(num_processes) as pool:
        print("grt")
        tasks = [(k, data, num_iterations, batch_size, device) for k in range(1, max_clusters + 1,50)]
        inertias = pool.starmap(kmeans_inertia, tasks)
    plt.figure(figsize=(10, 6))
    plt.plot(range(1, max_clusters + 1,50), inertias, marker='o')
    ...
    # plt.show()
    #save the plot
    plt.savefig(os.getcwd()+"/cluster/elbow_curve.png")
...
plot_elbow_curve(data, 1000, num_iterations=170, batch_size=1000, num_processes=4, device='cuda')